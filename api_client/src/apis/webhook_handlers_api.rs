...
/// struct for typed errors of method [`gitea_webhook_handler`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GiteaWebhookHandlerError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`github_webhook_handler`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GithubWebhookHandlerError {
    UnknownValue(serde_json::Value),
}
...
pub async fn gitea_webhook_handler(configuration: &configuration::Configuration, project_id: &str, body: &str) -> Result<models::WebhookHandlerSuccess, Error<GiteaWebhookHandlerError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WebhookHandlerSuccess`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WebhookHandlerSuccess`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GiteaWebhookHandlerError> = serde_json::from_str(&content).ok();
...
pub async fn github_webhook_handler(configuration: &configuration::Configuration, project_id: &str, body: &str) -> Result<models::WebhookHandlerSuccess, Error<GithubWebhookHandlerError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GithubWebhookHandlerError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
